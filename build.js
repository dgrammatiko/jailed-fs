...
const zipExtension = async (path, name, type) => {
  ...
  zip.writeZip(`site/dist/plg_system_restrictedfs_${version}.zip`);
}
...
(async function exec() {
  ...
  // let xmlContent = await getCurrentXml('src/package', 'pkg_wysiwygtinymce');
  // zip.addFile('pkg_wysiwygtinymce.xml', xmlContent);
